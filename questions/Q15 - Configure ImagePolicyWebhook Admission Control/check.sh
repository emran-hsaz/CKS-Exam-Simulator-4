...
score=0; total=4
hdr "Q15 | ImagePolicyWebhook Admission Control (4 pts)"

chk "ImagePolicyWebhook in --enable-admission-plugins" \
  "$(grep -q 'ImagePolicyWebhook' "$APISERVER" 2>/dev/null && echo true || echo false)" && ((score++))

chk "--admission-control-config-file is configured" \
  "$(grep -q 'admission-control-config-file' "$APISERVER" 2>/dev/null && echo true || echo false)" && ((score++))

chk "defaultAllow: false in admission-config.yaml" \
  "$(grep -q 'defaultAllow: false' /etc/kubernetes/admission-config.yaml 2>/dev/null && echo true || echo false)" && ((score++))

api_running=$(kubectl get pod -n kube-system -l component=kube-apiserver --no-headers 2>/dev/null | grep -c Running)
chk "API server is Running after config change" \
