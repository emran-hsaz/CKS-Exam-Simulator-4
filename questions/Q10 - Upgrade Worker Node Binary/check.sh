...
# Find the worker node (non-control-plane node)
...
chk "Worker node $WORKER is Ready" "$([ "$worker_status" = "Ready" ] && echo true || echo false)" && ((score++))

chk "Worker node $WORKER is not cordoned" \
  "$(echo "$worker_status" | grep -qv 'SchedulingDisabled' && echo true || echo false)" && ((score++))

cp_ver=$(kubectl get node --no-headers 2>/dev/null | grep -E 'control-plane|master' | awk '{print $5}' | head -1)
w_ver=$(kubectl get node "$WORKER" --no-headers 2>/dev/null | awk '{print $5}')
chk "Worker kubelet version matches control plane ($cp_ver)" \
  "$([ -n "$cp_ver" ] && [ "$cp_ver" = "$w_ver" ] && echo true || echo false)" && ((score++))

all_ready=$(kubectl get nodes --no-headers 2>/dev/null | awk '{print $2}' | grep -v Ready | wc -l)
chk "All cluster nodes are Ready" "$([ "$all_ready" = "0" ] && echo true || echo false)" && ((score++))
